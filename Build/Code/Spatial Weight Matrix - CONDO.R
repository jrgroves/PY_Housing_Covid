...
#Load main data and apply any limitations
  ...
  #Loop to pull the IDs and Distances for only previous observations
    ...
    save(ID, dist, ID0, ID2, file="./Build/Output/Condo_NN.RData")
  
#Create Sparse Spatial Matrix based on however nearest neighbors were chosen previously
  
 #load("./Build/Output/Condo_NN.RData") #load data if needed to be run separately
  load("./Build/Output/CoreData.RData")
  rm(main.s, main.s2)
  ...
  nn <- 50
    ...
    for(j in seq(1, nn)){
      m <- Matrix(nrow = 27429, ncol = 27429, data = 0, sparse = TRUE)
            for(i in seq(seed+1,length(ID0))){
              ifelse(is.na(unlist(ID0[[i]][j])),
                     next,
                     m[i, as.numeric(unlist(ID0[[i]][j]))] <- 1)
            }
      assign(paste0("s", j), m)
    }
    ...
    save.image("./Build/Output/Space_Condo.RData")
...
#Create Sparse Spatial Matrix based on however nearest neighbors were chosen previously
    ...
    for(j in seq(1, nn)){
      m <- Matrix(nrow = 27429, ncol = 27429, data = 0, sparse = TRUE)
      for(i in seq(seed+1,length(ID0))){
        ifelse(is.na(unlist(ID[[i]][j])),
               next,
               m[i, as.numeric(unlist(ID[[i]][j]))] <- 1)
      }
      assign(paste0("s0", j), m)
    }
    
    rm(list = grep("^s0", ls(), value = TRUE, invert = TRUE))
    
    save.image("./Build/Output/Space0_Condo.RData")
